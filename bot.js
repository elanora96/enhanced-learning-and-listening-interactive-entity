...
import { token, client_id, test_guild_id } from './config.json';
...
// @ts-expect-error idk this Client thing gave me a lot of greif
...
for (const module of slashCommands) {
  const commandFiles = readdirSync(`./interactions/slash/${module}`).filter(
    (file) => file.match(/\.(js|ts)$/),
  );

  for (const commandFile of commandFiles) {
    const command = import(`./interactions/slash/${module}/${commandFile}`);
...
for (const module of autocompleteInteractions) {
  const files = readdirSync(`./interactions/autocomplete/${module}`).filter(
    (file) => file.match(/\.(js|ts)$/),
  );
...
for (const folder of contextMenus) {
  ...
  for (const file of files) {
    const menu = import(`./interactions/context-menus/${folder}/${file}`);
...
for (const module of buttonCommands) {
  ...
  for (const commandFile of commandFiles) {
    const command = import(`./interactions/buttons/${module}/${commandFile}`);
...
for (const module of modalCommands) {
  const commandFiles = readdirSync(`./interactions/modals/${module}`).filter(
    (file) => file.match(/\.(js|ts)$/),
  );

  for (const commandFile of commandFiles) {
    const command = import(`./interactions/modals/${module}/${commandFile}`);
...
for (const folder of triggerFolders) {
  ...
  for (const file of triggerFiles) {
    const trigger = import(`./triggers/${folder}/${file}`);
